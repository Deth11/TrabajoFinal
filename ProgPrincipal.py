...
window = sg.Window('Sopa de Letras', default_element_size=(40, 1), grab_anywhere=False).Layout(p.layoutConfig(colores))
    ...
    if oficina != '':
        ...
        with open(oficina, "r") as log_file:
            ...
            for x in lista_de_temperaturas:
                aux.append(x['nombre'])
        w = sg.Window('Oficinas').Layout(p.layoutOficinas(aux))
        event, values = w.Read()
        for x in lista_de_temperaturas:
        ...
        if int(x['temperatura']) >= 20:
            sg.ChangeLookAndFeel('Reds')  # Cambia la estetica de la matriz a una mas veraniega
        else:
...
if button != 'Salir' and button is not None:
    ...
    while ok:
        if ColVer == ColSus or ColVer not in colores.values():
            if ColVer == ColSus:
                sg.Popup('Se eligieron 2 colores iguales, elije de nuevo', title='')
            wn = sg.Window('Eegir colores').Layout(p.layourElegirColor(colores))
            ...
            if ColSus == ColAdj:
                sg.Popup('Se eligieron 2 colores iguales, elije de nuevo', title='')
            wn = sg.Window('Elegir colores').Layout(p.layourElegirColor(colores))
            event, values = wn.Read()
            ColAdj = colores[values[2]]
            ColSus = colores[values[0]]
            ColVer = colores[values[1]]
...
if button == 'Listo' or button == 'Jugar':
    ...
    while ok:
        ...
        if int(values[0]) > 0 or int(values[1]) > 0 or int(values[2]) > 0:
            ...
            sg.Popup('Aumente al menos un tipo de palabra', title='Incorrecto')
        window.Close()
    cantV = int(values[0])
    cantS = int(values[2])
    cantA = int(values[1])
    ...
    try:
        try:
            if orientacion:
                ...
                g.hacer_grafico_vertical(matr, mx, caps, dicc, cantS, cantV, cantA,
                                     ColSus, ColVer, ColAdj, definiciones, cantPalIngresadas, fuente)
        except ValueError:
