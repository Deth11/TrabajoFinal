...
def hacer_grafico_horizontal(m, l, caps, dicc, CS, CV, CA, ColS, ColV, ColA, defi, cantP, fuente):
    ...
    layout = [
        ...
        [sg.Button('Verificar'), sg.Button('Exit'), sg.Text('                                  '), sg.Button('Ayuda')]
    ]
    window = sg.Window('Sopa de Letras').Layout(layout).Finalize()
    g = window.FindElement('_GRAPH_')
    #e = window.FindElement('_PALS_')
    ...
    while True:  # Event Loop
        ...
        if event == 'Ayuda':
            sg.Popup('Seleccione las palabras letra por letra y luego haga click en verificar', no_titlebar=True)
        if event == '_GRAPH_':
            if mouse == (None, None):
                continue
            ...
            for k in range(len(dicc[tipo][0])):
                if ing in m:
                    ...
                    if cont >= len(m):
                        ...
                        #e.Update('Cant. de plabras: ' + str(len(m) - cont))
...
def hacer_grafico_vertical(m, l, caps, dicc, CS, CV, CA, ColS, ColV, ColA):
    BOX_SIZE = 25

    layout = [
        [(sg.Button('Verbo', button_color=('black', ColV))),
         (sg.Button('Sustantivo', button_color=('black', ColS))),
         (sg.Button('Adjetivo', button_color=('black', ColA)))],
        [sg.Text('   '), sg.Text(CV),
         sg.Text('             '), sg.Text(CS),
         sg.Text('             '), sg.Text(CA)],
        [sg.Graph((l * 70, l * 70), (0, l * 55), (l * 55, 0), key='_GRAPH_',
                  change_submits=True, drag_submits=False)],
        [sg.Button('Verificar'), sg.Button('Exit')]
    ]
    ...
    while True:  # Event Loop
        ...
        if event == 'Verificar':
            if not tipo:
                continue
            s = ''
            for k in range(len(dicc[tipo])):
                if ing in m:
                    cont += 1
                    sg.Popup('Correcto ' + 'te faltan ' + str(len(m) - cont) + ' palabras', auto_close=True,
                             auto_close_duration=5)
                    break
                else:
                    sg.Popup('Selecciona de nuevo la plabra')
                    break
            ing = []
        if cont >= len(m):
            window.Close()
